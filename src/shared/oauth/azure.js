...
const authorize = async ({ nonce }) => {
  ...
  url.searchParams.set('nonce', nonce); // A value included in the request, generated by the app, that will be included in the resulting id_token as a claim
  ...
  const result = await launchWebAuthFlow({ url: decodedUrl }); // Result is an url which format is "<redirect_uri>#code=<code>&id_token=<id_token>"
...
export const oauthAzureLogin = async () => {
  ...
  const { code } = await authorize({ nonce });
  const { accessToken, idToken, scope } = await getAccessToken({ code });
  const claims = jwtDecode(idToken);
  if (!isValidIdToken(claims, nonce)) {
    return null;
  }
  const avatar = await getUserAvatar({ accessToken });
  const loginPayload = transformAzureLoginResp({ ...claims, idToken, scope, avatar });
